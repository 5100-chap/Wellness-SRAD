...
//Consigue insertar un nuevo registro de anuncio
router.put("/api/CrearAnuncio", verifyJWT, async (req, res, next) => {
    const request = new sql.Request();
    try {
        const result = await request.query(
            `EXEC [dbo].[CrearAnuncio] \'${req.body.fecha_inicio_evento}\', \'${req.body.fecha_fin_evento}\',\'${req.body.ubicacion}\',\'${req.body.descripcion}\',\'${req.body.duracion_inicio}\', \'${req.body.duracion_fin}\', \'${req.body.imagen}\', \'${req.body.titulo}\';`
        );
        res.sendStatus(200);
    } catch (err) {
        next(err);
