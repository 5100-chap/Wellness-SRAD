...
export class EditarAreaComponent implements OnInit {
    ...
    editarArea(
        ...
        var nombreFin : string = blankString(nombre) ? this.areaActual.NombreArea : nombre;
        var descFin : string = blankString(descripcion) ? this.areaActual.Descripcion : descripcion;
        var ubiFin : string = blankString(ubicacion) ? this.areaActual.Ubicacion : ubicacion;
        var matFin : string = blankString(material) ? this.areaActual.MaterialDisponible : material;
        
        var imaFin : string = blankString(imagen) ? this.areaActual.Imagen : imagen;
        var horaFinalF : string = blankString(horaFinal) ? this.areaActual.horaCierre : horaFinal;
        var horaInicioF : string = blankString(horaInicio) ? this.areaActual.horaApertura : horaInicio;

        this.apiService.editArea(id,nombreFin,descFin,ubiFin,matFin,imaFin,horaFinalF,horaInicioF).subscribe(error =>{
            console.log(error);
        });
        

        // this.apiService.editArea(id,nombre,descripcion,ubicacion,material,imagen,horaFinal,horaInicio).subscribe(error=>{
        //     console.log(error);
        // });
    }
    // crearArea(nombre: string, aforo: string, ubicacion: string, matDisp: string, horaFinal: string, horaInicio: string, imag: string) {
    ...
    //     var ubicacionFinal: string | null = esStringVacio(ubicacion) ? null : ubicacion;
    //     var horaFinalFinal: string | null = esStringVacio(horaFinal) ? null : horaFinal;
    //     var horaInicioFinal: string | null = esStringVacio(horaInicio) ? null : horaInicio;
    ...
    //     this.apiService.createArea(nombreFinal, null, 0, numAforo, ubicacionFinal, matDispFinal, true, null, null, imagFinal, horaFinalFinal, horaInicioFinal).subscribe(error => {
    //         console.log(error);
    //     });
    // }
